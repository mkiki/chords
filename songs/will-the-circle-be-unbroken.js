registerSong("will-the-circle-be-unbroken", {
  title: "Will the Circle Be Unbroken",
  author: "Ada R. Habershon, Charles H. Gabriel.",
  tuning: "openg",
  tempo: 120,
  chords: [ "G", "C", "D" ],
  tab: [
    { 
      name: "Verse/Chorus",
      signature: "2/4",
      measures: [
        {
          groups: [
            { notes: [ "Q", "E40", "S42", "S40P" ] },
          ]
        },
        {
          num: 1,
          groups: [
            { chord: "G", notes: [ "E30", "S10,S20,S30", "S50" ]},
            { notes: [ "E30", "S10,S20,S30", "S50" ]},
          ],
        },
        {
          groups: [
            { notes: [ "E40", "S10,S20,S30", "S50" ]},
            { notes: [ "S20", "S32+T", "S10,S20,S32", "S50" ]},
          ],
        },
        {
          groups: [
            { notes: [ "E30", "S10,S20,S30", "S50" ]},
            { notes: [ "S32", "S34S", "S10,S20,S34", "S50" ]},
          ],
        },
        {
          groups: [
            { notes: [ "E30", "S10,S20,S30", "S50" ]},
            { notes: [ "S20", "S32+T", "S10,S20,S32", "S50" ]},
          ],
        },
        {
          groups: [
            { chord: "C", notes: [ "E30", "S12,S21,S30", "S50" ]},
            { notes: [ "S40H", "S42", "S12,S21,S30", "S50" ]},
          ],
        },
        {
          groups: [
            { notes: [ "E30", "S12,S21,S30", "S50" ]},
            { notes: [ "S30", "S42+T", "S12,S21,S30", "S50" ]},
          ],
        },
        {
          groups: [
            { chord: "G", notes: [ "E40", "S10,S20,S30", "S50" ]},
            { notes: [ "S40H", "S42", "S42", "S40P" ]},
          ],
        },
        {
          groups: [
            { notes: [ "E30", "S10,S20,S30", "S50" ]},
            { notes: [ "S40H", "S42", "S42", "S40P" ]},
          ],
        },
        {
          groups: [
            { notes: [ "E30", "S10,S20,S30", "S50" ]},
            { notes: [ "E40", "S10,S20,S30", "S50" ]},
          ],
        },
        {
          groups: [
            { notes: [ "E30", "S10,S20,S30", "S50" ]},
            { notes: [ "S20", "S10+T", "S10,S20,S30", "S50" ]},
          ],
        },
        {
          groups: [
            { notes: [ "E10", "S10,S20,S30", "S50" ]},
            { notes: [ "S32", "S34S", "S10,S20,S34", "S50" ]},
          ],
        },
        {
          groups: [
            { notes: [ "E30", "S10,S20,S30", "S50" ]},
            { notes: [ "S30H", "S32", "S10,S20,S32", "S50" ]},
          ],
        },
        {
          groups: [
            { notes: [ "E20", "S10,S20,S30", "S50" ]},
            { notes: [ "S32", "S30P", "S10,S20,S30", "S50" ]},
          ],
        },
        {
          groups: [
            { chord: "D", notes: [ "S30H", "S32", "S10,S23,S32", "S50" ]},
            { notes: [ "E32", "S10,S23,S32", "S50" ]},
          ],
        },
        {
          groups: [
            { chord: "G", notes: [ "E30", "S10,S20,S30", "S50" ]},
            { notes: [ "S42", "S40P", "S10,S20,S30", "S50" ]},
          ],
        },
        {
          groups: [
            { notes: [ "Q30", "Q" ]},
          ],
        },
      ]
    },
  ],
  lyrics: [
    {
      type: "verse",
      text: [
        "(Verse) I was standing by my window,",
        "On one cold and cloudy day",
        "When I saw that hearse come rolling",
        "For to carry my mother away"
      ]
    },
    {
      type: "chorus",
      text: [
        "(Chorus) Will the circle be unbroken",
        "By and by, Lord, by and by",
        "There’s a better home a-waiting",
        "In the sky, Lord, in the sky"
      ],
    },
    {
      type: "verse",
      text: [
        "(Verse) I said to that undertaker",
        "Undertaker please drive slow",
        "For this lady you are carrying",
        "Lord, I hate to see her go"
      ]
    },
    {
      type: "struct", 
      text: [
        "(Chorus)"
      ]
    },
    {
      type: "verse",
      text: [
        "(Verse) Oh, I followed close behind her",
        "Tried to hold up and be brave",
        "But I could not hide my sorrow",
        "When they laid her in the grave"
      ]
    },
    {
      type: "struct", 
      text: [
        "(Chorus)"
      ]
    },
    {
      type: "verse",
      text: [
        "(Verse) I went back home, my home was lonesome",
        "Missed my mother, she was gone",
        "All of my brothers, sisters crying",
        "What a home so sad and lone"
      ]
    },
    {
      type: "struct", 
      text: [
        "(Chorus)"
      ]
    },
    {
      type: "verse",
      text: [
        "(Verse) We sang the songs of childhood",
        "Hymns of faith that made us strong",
        "Ones that mother Maybelle taught us",
        "Hear the angels sing along"
      ]
    },
  ]
});



